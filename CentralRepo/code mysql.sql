...
DELIMITER //
CREATE TRIGGER TRG_GIOHACANH
AFTER INSERT ON CHUYENBAY
FOR EACH ROW
BEGIN
   UPDATE CHUYENBAY
   SET GIOHACANH = ADDTIME(NEW.GIOKHOIHANH, SEC_TO_TIME(NEW.THOIGIANDUKIEN * 60))
   WHERE MACB = NEW.MACB;
END;//
DELIMITER ;
...
BEGIN
   ...
   SET C.SOGHEHANGTHUONGCONLAI = M.SOGHETHUONG, C.SOGHEHANGVIPCONLAI = M.SOGHEVIP
   WHERE C.MACB = NEW.MACB;
END;//
DELIMITER ;
...
BEGIN
   ...
   SELECT SOGHEVIP, SOGHETHUONG INTO tongghevip, tongghethuong
   FROM MAYBAY
   JOIN CHUYENBAY ON MAYBAY.MAMB = CHUYENBAY.MAMB
   WHERE CHUYENBAY.MACB = NEW.MACB;
...
DELIMITER //
CREATE PROCEDURE PRC_ThemMoi_NhanVien(IN in_MANV CHAR(8), IN in_TENNV VARCHAR(50), IN in_DIACHI CHAR(50), IN in_SDT VARCHAR(10), IN in_NGAYSINH DATE, IN in_NGAYVAOLAM DATE, IN in_GIOITINH CHAR(3), IN in_EMAIL VARCHAR(50), IN in_PASSWORD VARCHAR(50), IN in_NGAYTAOTK DATETIME, IN in_MAVT CHAR(8))
BEGIN
    IF EXISTS (SELECT * FROM NHANVIEN WHERE MANV = in_MANV) THEN
        SELECT 'Mã nhân viên đã tồn tại';
    ELSEIF NOT EXISTS (SELECT * FROM VAITRO WHERE MAVT = in_MAVT) THEN
        SELECT 'Mã vai trò không tồn tại';
    ELSE
        INSERT INTO NHANVIEN VALUES(in_MANV, in_TENNV, in_DIACHI, in_SDT, in_NGAYSINH, in_NGAYVAOLAM, in_GIOITINH, in_EMAIL, in_PASSWORD, in_NGAYTAOTK, in_MAVT);
    END IF;
END;//
DELIMITER ;
...
BEGIN
    IF EXISTS (SELECT 1 FROM KHACHHANG WHERE MAKH = in_MAKH) THEN
        SELECT 'Mã khách hàng đã tồn tại.';
